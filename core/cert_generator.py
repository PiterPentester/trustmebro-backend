...
class TrustMeBroCertificate:
    ...
    def create_certificate(
        ...
        if (
            "new year" in check_text
            or "новий рік" in check_text
            or "нового року" in check_text
            or "новому році" in check_text
        ):
        ...
        def create_top_elements(scale):
            ...
            def create_style(
                name, parent, fontSize, leading, spaceAfter, color=colors.black
            ):
            ...
            title_style = create_style(
                "TitleStyle", "Heading1", 28, 32, 0.5 * cm, colors.darkblue
            )
        ...
        if signature_img:
            signature_obj = Image(
                signature_img, width=(65.5 * 2) * mm, height=(5.7 * 2) * mm
            )
        ...
        template = PageTemplate(
            id="cert_template", frames=[top_frame, bottom_frame], onPage=draw_border
        )
