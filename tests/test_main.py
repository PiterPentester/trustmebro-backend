...
def test_validate_certificate_valid(mock_validator):
    ...
    response = client.get("/api/validate/12345")
...
def test_validate_certificate_invalid(mock_validator):
    ...
    response = client.get("/api/validate/invalid")
...
def test_download_certificate(mock_remove, mock_path):
    ...
    with patch("builtins.open", new_callable=MagicMock) as mock_open:
        ...
        response = client.get("/api/download/12345")

        assert response.status_code == 200
        assert response.headers["content-type"] == "application/pdf"
        assert (
            response.headers["content-disposition"] == "attachment; filename=12345.pdf"
        )
